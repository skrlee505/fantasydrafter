...
- Known scheduled byes count as zero. Missing, rank-only, and stale projection rows remain unknown. A missing projection withholds the numerical verdict when it belongs to a traded player, prevents a legal lineup from being filled, or makes a required roster-drop comparison unresolved. Unrelated bench gaps are retained as named caveats rather than invalidating the whole analysis.
...
Sleeper league endpoints supply ownership, scoring, standings, and matchups. Its schedule endpoint establishes completed games and byes. Its statistics host distributes weekly projections and completed-game stats, with the company field retained for attribution. Free nflverse weekly player statistics add completed-game carries, targets, target share, air-yard share, and scoring fields. The DynastyProcess fantasy-player ID table maps Sleeper IDs to nflverse GSIS IDs. nflverse rows merge with, rather than replace, available Sleeper snap and red-zone fields; failure leaves Sleeper usage intact and adds a warning.

League refresh caches for 15 seconds; schedule and season state for one minute; projections, Sleeper statistics, and nflverse player statistics for six hours; the player directory and DynastyProcess ID map for one day. A projection's publication timestamp remains distinct from retrieval. Seven days is the hard projection age cutoff, with a displayed 24-hour review target. Refreshing an offline snapshot cannot restore trust. Saved snapshots preserve the original scorer version, rules, projections, and preferences.
